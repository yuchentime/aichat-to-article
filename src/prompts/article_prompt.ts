...
1. 逐行读取 {Message Chunk} 内的对话记录，理解当前用户的诉求或反馈:
  
  a. **诉求：** 用户期望 AI 接下来给出的回复
  
  b. **反馈：** 用户针对 AI 已经给出的回复的反馈意见
  
2. 根据用户的诉求或反馈，从 AI 给出的回复中提取相关内容。值得注意的是，提取的内容需要满足如下条件:
  
  a. **语义压缩：** 不会破坏原有含义。
  
  b. **具体内容：** 针对用户诉求或反馈给出的直接指导方案，如代码、引用链接、公式、参数等。这些内容一旦被压缩，会直接导致信息失真，从而严重影响用户的理解。
  
  c. **相关性：** 用户主动认可的内容必须保留；虽然用户没有主动提及，但是能起到辅助性理解作用的内容也务必保留。
  
3. 检索上次提取的内容 {Last Result} ，是否存在和当前提取内容的重叠部分。如果存在重叠部分，例如针对相同方案的修正或补充，则使用当前内容覆盖更新，并整合剩余部分内容。
  
4. 最终整合输出每轮对话中用户的诉求或反馈，以及从 AI 回复中提取到的相关内容
  
5. 以全局视角检查最终整合结果，是否存在上下文关联性的缺失，导致对提取内容的误判。如有，补充遗漏内容。
